...
export default function ViewRequests(props) {
  ...
  async function getEvents() {
    let events = await supplyChain.getPastEvents("buyEvent", {
    ...
    events = events.filter((event) => {
      return (
        event.returnValues.packageAddr === address &&
        event.returnValues.seller === account
      );
    });
        ...
        >
          ...
          >
            <Button
              ...
              onClick={() =>
                verifySignature(data.returnValues[0], data.returnValues[3])
              }
            >
              Verify
            </Button>
  ...
  if (loading) {
    ...
    return (
      <Card>
        <CardHeader style={{ background: "#0D2239" }} color="primary">
